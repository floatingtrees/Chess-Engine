...
def pins_and_checks_search(position, position_swapped, white, king_location_y, king_location_x):
	...
	if white:
		for i in range(-1, 2, 2):
			...
			print(search_square_pawn)
			if search_square_pawn in position_swapped:
				print("SECOND", search_square_pawn)
...
def king_moves_search(legal_moves, position, position_swapped, piece, white):
	...
	for direction in queen_moves:
		...
		if move_x >= 0 and move_x <= 7 and move_y >= 0 and move_y <= 7:
			moving_into_check = pins_and_checks_search(position, position_swapped, white, move_y, location_x)[1]
			if (move_y, move_x) not in position_swapped:
...
def legal_move_search(position, position_swapped, white, previous_move): # If in check, only consider moves that end on top of the check square
	try:
		if white:
			king_location_y, king_location_x = position["K0"]
		else:
...
if __name__ == "__main__":
	...
	assert white_pieces == ("K", "Q", "R", "B", "N", "P"), "White pieces changed"
	assert black_pieces == ("k", "q", "r", "b", "n", "p"), "Black pieces changed"
